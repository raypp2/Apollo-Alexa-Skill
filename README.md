...
8) For `Alexa.ChangeReport` / `AcceptGrant` (Stage 7's ChangeReport leg), set `ALEXA_CLIENT_ID` and `ALEXA_CLIENT_SECRET` to the Login with Amazon "Send Alexa Events" security profile credentials (Alexa Developer Console -> your skill -> Permissions). AcceptGrant exchanges the account-linking grant code for an access/refresh token pair and stores it as a SecureString JSON parameter at `/apollo/alexa/eventgateway-tokens` in SSM Parameter Store (`us-east-1`); changeReport.mjs reads and refreshes it from there before POSTing to the Event Gateway. The Lambda's execution role needs `ssm:GetParameter` and `ssm:PutParameter` on that parameter. An IoT Rule (configured outside this repo) invokes this same Lambda with a non-directive `{thingName, reported, previous}` event on shadow updates; `CHANGE_REPORTS_ENABLED` in `handleDiscovery.mjs` stays `false` until the token flow has been verified live, after which flipping it requires a device re-discovery ("Alexa, discover devices").

## Local development / tests

This repo now has a `package.json` (`npm test` runs `node --test`) so ReportState, ChangeReport, AcceptGrant, and Discovery logic can be unit tested with injected fakes instead of hitting AWS. `npm install` before running `npm test`.

**Deploy note:** the repo previously shipped as bare `.mjs` files with no `package.json`, relying on whichever `@aws-sdk/client-*` packages the Lambda console's Node.js managed runtime happens to bundle. That's undocumented and AWS explicitly recommends against relying on it (see [aws-sdk-js-v3's Lambda guidance](https://github.com/aws/aws-sdk-js-v3/blob/main/supplemental-docs/AWS_LAMBDA.md)) — only bundle-your-own-SDK is guaranteed. `@aws-sdk/client-sqs`/`client-s3` have apparently been fine bundled so far, but `@aws-sdk/client-iot-data-plane` (added for ReportState) and `@aws-sdk/client-ssm` (added for AcceptGrant/ChangeReport) are not commonly-bundled clients and should not be assumed present. Having a `package.json` doesn't break a console/zip deploy of bare `.mjs` files by itself — but to safely use ReportState/ChangeReport/AcceptGrant, zip `node_modules` (after `npm install`) together with the `.mjs` files and `package.json` rather than editing/uploading source only through the inline console editor.